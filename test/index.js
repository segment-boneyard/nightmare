...
describe('Nightmare', function () {
  ...
  describe('navigation', function () {
    ...
    it('should fail if navigation target is invalid', function () {
      return nightmare.goto('http://this-is-not-a-real-domain.tld').then(
        ...
        function (err) {
          assert.strictEqual(err instanceof Error, true)
          assert.strictEqual(err.message, 'navigation error')
          assert.strictEqual(err.details, 'ERR_NAME_NOT_RESOLVED')
          assert.strictEqual(err.code, -105)
          assert.strictEqual(err.url, 'http://this-is-not-a-real-domain.tld/')
        }
      )
    })
  ...
  describe('manipulation', function () {
    ...
    it('should inject javascript onto the page', function* () {
      ...
      var numAnchors = yield nightmare
        .goto(fixture('manipulation'))
        .inject('js', 'test/files/jquery-3.6.0.min.js')
        .evaluate(function () {
  ...
  describe('cookies', function () {
    ...
    it('should return a proper error', function* () {
      try {
        ...
        assert.strictEqual(e.message, 'Setting cookie failed')
      }
    })
  })
  ...
  describe('options', function () {
    ...
    it('should wait and fail with waitTimeout with queued functions', function () {
      nightmare = Nightmare({ waitTimeout: 254 })
      return nightmare.goto(fixture('navigation')).wait('foobar').exists('baz')
        .should.be.rejected
    })
  ...
  describe('security', function () {
    ...
    it('should not expose the ipc to the 3rd party', function* () {
      ...
      nightmare.on(
        'console',
        function (_type, ipc, windowIPC, send, windowSend) {
