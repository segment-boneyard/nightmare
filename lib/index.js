...
Nightmare.prototype.setupInstance = function(done) {
  ...
  if (PHANTOMJS_INITING) {
    ...
    var self = this;
    var flags = SUPPORTED_ARGS.reduce(function(flags, option) {
      if (self.options[option] !== null) {
        flags.push('--' + option.replace(/([A-Z])/g, '-$1').toLowerCase()
          + '=' + self.options[option]);
      }
      return flags;
    }, []);
    
    flags.push({ port: port, dnodeOpts: dnodeOpts });
    flags.push(function(instance) {
      PHANTOMJS_INSTANCE = instance;
      done(instance);
    });
    phantom.create.apply(phantom, flags);
  }
};
