...
Nightmare.prototype._wait = function(options, cb) {
  ...
  if (typeof options === 'number') {
    var ms = options;
    debug('Waiting for a fixed time: ' + ms + 'ms');
    setTimeout(cb, ms);
  }
  else if (typeof options === 'string') {
    var selector = options;
    debug('Waiting for the element ' + selector);
    ...
    debug('Waiting for the next page load.');
...
Nightmare.prototype._screen = function(path, cb) {
  debug('Taking a screenshot: ' + path);
...
Nightmare.prototype._setup = function (cb) {
  ...
  this.setupInstance(function (err, instance) {
    ...
    instance.createPage(function(err, page) {
      ...
      page.setViewport(self.options.viewport, function (err) {
        if (err) return cb(err);
        debug('Set viewport.');
        cb();
      });
