...
Nightmare.prototype.use = function(plugin, cb){
  debug('.use()-ing a plugin');
  var self = this;
  // We need to "insert" the plugin's functions
  // at the beginning of the queue.
  var cache = self._queue;
  self._queue = [];
  plugin(self);
  // ...then replace all the later functions at the end.
...
Nightmare.prototype.screenshot = function(path, cb) {
...
Nightmare.prototype.useragent = function(useragent, cb) {
  debug('.useragent() to ' + useragent);
  this.page.set('settings.userAgent', useragent, cb);
};
...
Nightmare.prototype.run = function(callback) {
  ...
  var recurse = function (err) {
    if (err || self._queue.length === 0) return callback(err, self);
    debug('.run() recursing with queue length ' + self._queue.length);
...
createMethod('wait');
createMethod('setup');
createMethod('screenshot');
createMethod('evaluate');
createMethod('viewport');
createMethod('useragent');