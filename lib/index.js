...
function Nightmare (options, args) {
  if (!(this instanceof Nightmare)) return new Nightmare(options, args);
  this.args = args || [];
...
Nightmare.prototype.setupInstance = function(done) {
  ...
  if (PHANTOMJS_INITING) {
    ...
    this.args.unshift({ port: port, dnodeOpts: dnodeOpts });
    this.args.unshift(function(instance) {
      PHANTOMJS_INSTANCE = instance;
      done(instance);
    });
    phantom.create.apply(phantom, this.args);
  }
};
