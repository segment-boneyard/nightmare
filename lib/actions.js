 ...
 *
 * @param {Function} callback If return a new cookie object, will update cookies
 * @param {Function} done
 */

exports.cookies = function(callback, done){
  debug('.cookie()');
  var self = this;
  this.page.get('cookies', function(cookies){
    var newCookies = callback(cookies);
    if(newCookies){
      self.page.set('cookies', newCookies, done);
    }else{
      done();
    }
  });
