...
/**
 * Go to a new url, using POST method.
 *
 * @param {String} url
 * @param {Function} done
 */

exports.gotoPost = function(url, postData, done) {
  debug('.gotoPost() url: ' + url + ', with data: ' + postData);
  this.page.open(url, 'POST', postData, function(status) {
    debug('.gotoPost() page loaded: ' + status);
    setTimeout(done, 500);
  });
};
