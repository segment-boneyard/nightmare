...
function waitelem (self, selector, done) {
  ...
  var newDone = function(err) {
    if (err) {
      if (self.options.screenshotOnError && self.options.screenshotOnErrorFolder) {
        var screenshotPath = path.join(self.options.screenshotOnErrorFolder, `${new Date().getTime()}.png`);
        exports.screenshot.apply(self, [screenshotPath, function (err) {
          var msg = err 
            ? `Failed to save file: ${err.message || err}`
            : `Successfully saved at ${screenshotPath}`;
          debug(`screenshot of .wait() for ${selector}: ${msg}`);
