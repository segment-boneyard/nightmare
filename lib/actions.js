...
exports.visible = function(selector, callback, done) {
  ...
  this.page.evaluate(function(selector) {
    var elem = document.querySelector(selector),
      style, isSizeGreaterThanZero, isElementDisplayed,
      isElementVisible, isElementOpaque, isTypeNotHidden;
    if (!elem) {
      return false;
    }
    style = window.getComputedStyle(elem);
    isSizeGreaterThanZero = elem.offsetWidth > 0 && elem.offsetHeight > 0;
    isElementDisplayed = style.display !== 'none';
    isElementVisible = ['hidden', 'collapse'].indexOf(style.visibility) === -1;
    isElementOpaque = style.opacity > 0;
    isTypeNotHidden = elem.getAttribute('type') !== 'hidden';
    return isSizeGreaterThanZero && isElementDisplayed &&
      isElementVisible && isElementOpaque && isTypeNotHidden;
  }, function(result) {
    callback(result);
    done();
