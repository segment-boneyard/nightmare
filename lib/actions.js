...
exports.type = function () {
  ...
  focusSelector.bind(this)(function (err) {
    ...
    if ((text || '') === '') {
      ...
      const dispatchKeyEvent = self.chrome.Input.dispatchKeyEvent;
      ...
      function type() {
        ...
        dispatchKeyEvent({type: 'keyDown', text: ch});
...
exports.viewport = function (width, height, done) {
  ...
  this.chrome.Target.targetCreated(target => {
    this.chrome.debug('Target.targetCreated', target);

    if (target.type === 'browser') {
      this.chrome.target = target;

      this.chrome.Browser.getWindowForTarget({targetId: target.targetId})
        .then(window => {
          this.chrome.debug('Browser.getWindowForTarget', window);
          delete this.chrome.client._events['Browser.getWindowForTarget'];
          delete this.chrome.client._events['Target.targetCreated'];
          this.chrome.window = window;

          this.chrome.Browser
            .setWindowBounds({
              windowId: this.chrome.window.windowId,
              bounds: {width, height}
            });
        });
    }
...
exports.useragent = function (userAgent, done) {
  ...
  this.chrome.Network.setUserAgentOverride({userAgent})
    .then(() => done());
};
...
exports.screenshot = function (path, clip, done) {
  ...
  debug('.screenshot()', clip && {clip});

  this.chrome.Page.captureScreenshot(clip && {clip})
    .then(img => {
      const buf = new Buffer(img.data, 'base64');
      debug('.screenshot() captured with length %s', buf.length);

      if (typeof path === 'function' || !path) {
        done(null, buf);
      } else {
        fs.writeFile(path, buf, 'base64', err => {
          if (err) {
            done(err);
          } else {
            done(null, buf);
...
exports.pdf = function (path, options, done) {
  ...
  this.chrome.Page.printToPDF(options)
    .then(pdf => {
      var buf = new Buffer(pdf.data, 'base64');
      debug('.pdf() captured with length %s', buf.length);
      path ? fs.writeFile(path, buf, 'base64', done) : done(null, buf);
    })
    .catch(err => {
      done(err);
    });
...
exports.cookies.get = function (name, done) {
  ...
  this.chrome.Network.getCookies()
    .then(cookies => {
      this.chrome.debug('Network.getCookies', cookies);
      ...
      if (typeof name === 'string') {
        ...
        name(undefined, cookies.cookies);
      }
    })
    .catch(err => done(err));
};
...
exports.cookies.set = function (name, value, done) {
  ...
  Promise.all(cookies.map(cookie => {
    cookie.url = this.currentUrl;
    return this.chrome.Network.setCookie(cookie);
  }))
  .then(() => done())
  .catch(err => done(err));
};
...
exports.cookies.clear = function (name, done) {
  ...
  (arguments.length > 1 ? Promise.all(cookies.map(name => {
    const cookie = {
      cookieName: name,
      url: this.currentUrl
    };

    return this.chrome.Network.deleteCookie(cookie);
