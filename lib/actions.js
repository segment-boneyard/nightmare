...
exports.wait = function () {
  ...
  // change args in place to remove done()
  var done = args.splice(-1,1)[0];
  if (args.length < 1) {
  ...
  if (typeof arg === 'number') {
    ...
    var timeout = null;
    if (args.length > 1 && typeof args[args.length -1] === 'number') {
      timeout = args.splice(-1,1);
    }
    debug('.wait() for '+args+' element(s)'+(timeout ? ' or '+timeout+'msec' : ''));
    waitelem.apply({ timeout: timeout }, [this, ...args , done]);
  }
...
function waitelem () {
  ...
  debug('Waiting for selectors: ' + jsesc(args) );
  eval(`var elementPresent = function() {
    var selectors = ${jsesc(args)};
    return selectors.some( function(selector){
      return !!( document.querySelector(selector) );
    })
  }`);
  // eval("var elementPresent = function() {"+
  //     "  return !!(document.querySelector('"+jsesc(args[0])+"'));"+
  //     "};");
