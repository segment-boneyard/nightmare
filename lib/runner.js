 ...
 */
 if (processArgs['proxy-username-and-password']) {
     var splitUsernameAndPassword = processArgs['proxy-username-and-password'].split(':');
     app.on('login', function(event, webContents, request, authInfo, callback) {
         console.log('Login event fired');
         event.preventDefault();
         callback(splitUsernameAndPassword[0], splitUsernameAndPassword[1]);
     })
 }
