...
const {app, ipcMain, BrowserWindow, powerSaveBlocker, protocol} = require('electron');
...
app.on('ready', function () {
    ...
    parent.respondTo('browser-initialize', function (opts, done) {
        ...
        ipcMain.on('page', function (sender/*, arguments, ... */) {
    ...
    parent.respondTo('javascript', function (code, isAsync, done) {
        ...
        ipcMain.on('log', logForwarder);

        win.webContents.executeJavaScript(code, function (result) {

            ipcMain.removeListener("log", logForwarder);

            if (isAsync) {
                //wait until the script emits a 'javascript' event.
                ipcMain.once('javascript', function (event, result) {
                    if (result.err) {
    ...
    parent.respondTo('electronAction', function (name, fntext, done) {
        ...
        for (let action of actions) {
            ...
            fn(name, options, parent, win, ipcMain, frameManager);
        }
        done.resolve();
    });
...
function forward(name) {
    return function (event) {
        parent.emit.apply(parent, [name, event].concat(Array.from(arguments).slice(1)));
    };
}
