...
app.on('ready', function() {
  ...
  parent.on('browser-initialize', function(options) {
    var userPreload = resolve('userPreloadTemplate.js');
    if(options['web-preferences'] && options['web-preferences'].preloadPath){
      userPreload = options['web-preferences'].preloadPath;
    }
    fs.writeFileSync(join(app.getPath('userData'), '__preload.js'), preload({userPreloadFilepath: userPreload}));
    parent.emit('log', join(app.getPath('userData'), '__preload.js'));
