...
app.on('ready', function () {
    ...
    parent.on('browser-initialize', function (options) {
        ...
        renderer.on('page', function (sender/*, arguments, ... */) {
            parent.emit.apply(parent, ['page'].concat(_.slice(arguments, 1)));
        });
     ...
     * Continue
     */
    parent.on("waitUntilStopLoading", function () {
        ...
        function ready() {
            parent.emit('waitUntilStopLoading');
        };
    });
...
function forward(event) {
    return function () {
        parent.emit.apply(parent, [event].concat(_.slice(arguments)));
    };
}
