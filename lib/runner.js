...
app.on('ready', function() {
  ...
  /**
   * Add custum functionality
   */

  parent.on('plugin', function(name, fntext){
    var fn = new Function('with(this){ return ' + fntext + '}').call({require:require});
    fn(name, parent, win, renderer, function(err){
      parent.emit('plugin', err);
     });
  });
