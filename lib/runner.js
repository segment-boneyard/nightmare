...
if (process.argv.length > 2) {
  ...
  if (processArgs.proxy){
    app.commandLine.appendSwitch('proxy-server', processArgs.proxy);
  }
  if (processArgs.paths) {
    for (var i in processArgs.paths) {
      app.setPath(i, processArgs.paths[i]);
    }
  }
}
