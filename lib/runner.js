...
app.on('ready', function () {
     ...
     * javascript
     */
    parent.on('javascript', function (code, isAsync, responseId) {
        ...
        var parentMessageName = "javascript";
        if (responseId)
            parentMessageName += " " + responseId;

        parent.emit("log", parentMessageName);
        win.webContents.executeJavaScript(code, function (result) {

            renderer.removeListener("log", logForwarder);

            //If we're not async, we're done at this point.
            if (!isAsync) {
                parent.emit(parentMessageName, result);
                return;
            }

            //otherwise, wait until the script emits a 'javascript' event.
     ...
     * insert css
     */
    parent.on('insertCSS', function (css) {
        win.webContents.insertCSS(css);
        parent.emit("insertCSS");
    });

    /**
     * Add custom functionality
     */
    parent.on('electronAction', function (name, fntext, id) {
        var fn = new Function('with(this){ parent.emit("log", "registering electron action for ' + name + '"); return ' + fntext + '}')
            .call({
                require: require,
                parent: parent
            });
