...
app.on('ready', function() {
  ...
  parent.on('browser-initialize', function(options) {
    var userPreload='';
    if(options['web-preferences'] && options['web-preferences'].preload){
      userPreload = fs.readFileSync(options['web-preferences'].preload);
    }
    fs.writeFileSync(join(app.getPath('userData'), '__preload.js'), preload({src: userPreload}));
    ...
    renderer.on('page-prompt', function(event, message, response){
      parent.emit('page-prompt', message, response);
    });
    renderer.on('page-confirm', function(event, message, response){
      parent.emit('page-confirm', message, response);
    });
