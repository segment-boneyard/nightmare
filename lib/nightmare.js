...
class Nightmare {
    ...
    _invokeRunnerOperation(operationName) {
        ...
        var p = new Promise(function (resolve, reject) {
            ...
            child.once(operationName, function (result) {
    ...
    chain() {
        ...
        let initializeChainablePromise = function (chainablePromise) {
            ...
            for (let functionName of getFunctionNames(self)) {
                chainablePromise[functionName] = wrap(functionName, self[functionName]);
            }
            ...
            if (_.isObject(Nightmare._namespaces)) {
                for (let ns of Nightmare._namespaces) {
                    if (_.isObject(self[ns])) {
                        chainablePromise[ns] = {};
                        for (let functionName of getFunctionNames(self[ns])) {
                            chainablePromise[ns][functionName] = wrap(ns + "." + functionName, self[ns][functionName]);
                        }
                    }
                }
            }
        ...
        initializeChainablePromise(initialPromise);
    ...
    initializeNamespaces() {
        ...
        for (let ns of Nightmare._namespaces) {
            if (_.isObject(self[ns])) {
                for (let functionName of Object.getOwnPropertyNames(self[ns])) {
                    if (_.isFunction(self[ns][functionName])) {
                        self[ns][functionName] = self[ns][functionName].bind(self);
                    }
                }
            }
        }
    ...
    initializeElectronActions() {
        ...
        for (let electronActionName of _.keys(Nightmare._electronActions)) {
            ...
            var p = this._invokeRunnerOperation("electronAction", electronActionName, String(Nightmare._electronActions[electronActionName]));
            promises.push(p);
        }
    ...
    static action(name) {
        ...
        var fn, ns, electronFn, electronNs;
