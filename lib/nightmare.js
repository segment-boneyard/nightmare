...
class Nightmare {
    ...
    *click(selector) {
     ...
     * @param {String} selector
     */
    *clickAndWaitForFinishLoad(selector) {
        debug('.clickAndWaitForFinishLoad() on ' + selector);
    ...
    *emulateClick(selector) {
    ...
    /**
     * Click an element and wait until the navigation operation completes.
     * Use this function when you expect a click to perform a navigation action, usually on anchor elements, but elsewhere too. 
     * @param {String} selector
     */
    *clickAndWaitUntilNavigationCompletes(selector) {
        debug('.clickAndWaitUntilNavigationCompletes() on ' + selector);
        yield this.evaluate_now(function (selector) {
            document.activeElement.blur();
            var element = document.querySelector(selector);
            var event = document.createEvent('MouseEvent');
