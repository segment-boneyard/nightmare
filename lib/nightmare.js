...
class Nightmare {
            ...
            if (_.isObject(Nightmare._namespaces)) {
                for (let ns of Nightmare._namespaces) {
                    if (_.isObject(self[ns])) {
                        chainablePromise[ns] = function () {
                        };

                        for (let functionName of getFunctionNames(self[ns].prototype)) {
                            chainablePromise[ns][functionName] = wrap(ns + "." + functionName, self[ns].prototype[functionName]);
                        }
                    }
                }
            }
    ...
    static action(name) {
        ...
        if (ns) {
            ...
            nsObj.prototype.constructor = nsObj;
