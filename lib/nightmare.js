...
class Nightmare {
    ...
    initializeNamespaces() {
        ...
        for (let ns of Nightmare._namespaces) {
            if (ns.bindFunctions && _.isFunction(self[ns.name])) {
                self[ns.name] = new self[ns.name](self);
                //Ensure that this is bound to the Nightare object on all namespace functions.
                for (let functionName of Object.getOwnPropertyNames(Object.getPrototypeOf(self[ns.name]))) {
                    if (_.isFunction(self[ns.name][functionName]) && functionName !== "constructor") {
                        self[ns.name][functionName] = self[ns.name][functionName].bind(self);
                    }
                }

                self.initializeActionTuples(self[ns.name]);
            }
        }
    };
     ...
     */
    static registerNamespace(name, bindFunctions) {
        if (_.isUndefined(Nightmare._namespaces))
            Nightmare._namespaces = [];

        bindFunctions = bindFunctions || true;
        ...
        if (!_.find(Nightmare._namespaces, { name: nsObj.name })) {
            ...
            throw util.format("A namespace for %s has already been registered.", nsObj.name);
        }
    };
}
