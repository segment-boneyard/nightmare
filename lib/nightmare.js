...
Nightmare.prototype.plugin = function(ns, hash){
  if(hash.nightmareAction) {
    Nightmare.prototype[ns] = queued(ns, hash.nightmareAction);
  }
  if(hash.electronAction) {
    queued('plugin', function(ns, hash, done){
      this.child.once('plugin', done);
      this.child.emit('plugin', ns, String(hash.electronAction));
    }).bind(this)(ns, hash)
  }
