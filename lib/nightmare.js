...
function Nightmare(options) {
  ...
  this.child.once('ready', function() {
    this.child.once('browser-initialize', function() {
      this.state = 'ready';
    }.bind(this));
    this.child.emit('browser-initialize', options);
  }.bind(this));
...
Nightmare.prototype.run = function(fn) {
  ...
  var next = function (err, res) {
    ...
    if (!item) return done.apply(this, arguments);
    var args = item[1] || [];
    var method = item[0];
  ...
  var after = function (err, res) {
    ...
    this.child.once('continue', function() {
      next.apply(this, args);
    });
    this.child.emit('continue');
  }.bind(this)
