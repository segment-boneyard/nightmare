...
/** 
 * Extend the Nightmare instance, Electron instance, or both.
 *
 * @param {String} name
 * @param {Function} hash.nightmareAction
 * @param {Function} hash.electronAction
 */

Nightmare.prototype.plugin = function(name, hash){
  if(hash.nightmareAction) {
    this[name] = queued(name, hash.nightmareAction);
  }
  if(hash.electronAction) {
    queued('plugin', function(name, hash, done){
      this.child.once('plugin', done);
      this.child.emit('plugin', name, String(hash.electronAction));
    }).bind(this)(name, hash)
  }
