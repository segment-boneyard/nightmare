...
var log = require('debug')('nightmare:log'); // eslint-disable-line import/order
var debug = require('debug')('nightmare'); // eslint-disable-line import/order
...
var source = require('function-source'); // eslint-disable-line
var proc = require('child_process');
var actions = require('./actions'); // eslint-disable-line import/order
var path = require('path');
var sliced = require('sliced');
var child = require('./ipc'); // eslint-disable-line import/order
...
Nightmare.prototype.run = function (fn) {
  ...
  // next function
  function next(err, res) { // eslint-disable-line no-unused-vars
  ...
  function after(err, res) { // eslint-disable-line no-unused-vars
    err = err || self.die;
...
Nightmare.prototype.evaluate_now = function (js_fn, done) {
  ...
  var source = template.execute({src: String(js_fn), args});
...
Nightmare.prototype.queue = function (done) { // eslint-disable-line no-unused-vars
  if (!arguments.length) {
    return this._queue;
  }
  var args = sliced(arguments);
...
function queued(name, fn) {
  return function action() { // eslint-disable-line func-names
