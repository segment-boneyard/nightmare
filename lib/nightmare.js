...
//console.log(default_electron_path);
// able to log.
...
// shall add random args to here.
  ...
  this.options = options
	function kongfu(fu){
for (let kong in fu){
//	console.log(kong,typeof(kong));
	//	this is string
if(typeof(fu[kong])===typeof("string")){
	//console.log("string");
}else{
...
var arg_debug=[runner].concat(JSON.stringify(electronArgs))
	if(options.cli_args){
		if (typeof(options.cli_args)===typeof([])){
			if (kongfu(options.cli_args)){
		arg_debug=options.cli_args.concat(arg_debug)}else{
const error = new Error("cli_args must be an array of string.");
throw error
		}
		}
		}
		//full of string.
//	arg_debug.unshift("--no-sandbox")
//	console.log(arg_debug,typeof(arg_debug))
