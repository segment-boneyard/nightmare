...
class Nightmare {
    ...
    *back() {
        debug('.back()');
        return yield this._invokeRunnerOperation("goBack");
    };
    ...
    *clickAndWaitUntilFinishLoad(selector) {
        ...
        var child = this.child;
        var p = this._invokeRunnerOperation("waitUntilFinishLoad");
    ...
    *emulateClick(y, x) {
        ...
        if (_.isString(y) && !x) {
            var offset = yield this.getElementOffset(y);

            x = Math.floor(offset.left + (offset.width / 2));
            y = Math.floor(offset.top + (offset.height / 2));
            return yield this._invokeRunnerOperation("emulateClick", { x: x, y: y });
        }
        //just pass the full object
        else if (_.isObject(y) && !x) {
            return yield this._invokeRunnerOperation("emulateClick", y);
        }
        //click x, y.
        else {
            return yield this._invokeRunnerOperation("emulateClick", { x: x, y: y });
    ...
    *emulateKeystrokes(selector, text, opts) {
        ...
        if (selector) {
            yield this.emulateClick(selector);
        ...
        return yield this._invokeRunnerOperation("emulateKeystrokes", { keyCodes: keyCodes, finalKeystrokeDelay: opts.finalKeystrokeDelay });
    }
    ...
    evaluate_now(js_fn) {
        ...
        return this._invokeRunnerOperation('javascript', template.execute({ src: String(js_fn), args: argsList }));
    }
    ...
    *forward() {
        ...
        return yield this._invokeRunnerOperation("goForward");
    };
    ...
    *goto(url, headers) {
        ...
        return yield this._invokeRunnerOperation("goto", url, headers);
    };
    ...
    *getElementOffset(selector) {
    ...
    init() {
        ...
        child.on('login', function () { log('login', JSON.stringify(Array.from(arguments))); });
        child.on('media-started-playing', function () { log('media-started-playing', JSON.stringify(Array.prototype.slice.call(arguments))); });
        child.on('media-paused', function () { log('media-paused', JSON.stringify(Array.prototype.slice.call(arguments))); });
    ...
    *inject(type, file) {
        ...
        if (type === 'js') {
            ...
            return yield this._invokeRunnerOperation("javascript", template.inject({ src: js }));
        }
        else if (type === 'css') {
            var css = fs.readFileSync(file, { encoding: 'utf-8' });
            return yield this._invokeRunnerOperation("css", css);
        }
        else {
    ...
    *insert(selector, text) {
        ...
        if (!text) {
            ...
            return yield this._invokeRunnerOperation("insert", text);
        }
    }
    ...
    *pdf(path, options) {
        ...
        return yield this._invokeRunnerOperation("pdf", path, options)
            .then(function (pdf) {
    ...
    *reload() {
        debug('.reload()');
        return yield this._invokeRunnerOperation("reload");
    }
    ...
    *screenshot(path, clip) {
        ...
        return yield this._invokeRunnerOperation("screenshot", clip)
            .then(function (img) {
    ...
    *scrollTo(y, x) {
        ...
        if (!x && _.isString(y)) {
            var result = yield this.evaluate_now(function (selector) {
                var element = document.querySelector(selector);
                if (element) {
    ...
    *setAudioMuted(isMuted) {
        ...
        return yield this._invokeRunnerOperation("audio", isMuted);
    };
    ...
    *setAuthenticationCredentials(username, password) {
        ...
        return yield this._invokeRunnerOperation("setAuthenticationCredentials", username, password);
    };
    ...
    *stop() {
        debug('.stop()');
        return yield this._invokeRunnerOperation("stop");
    }
    ...
    *title() {
        ...
        return yield this._invokeRunnerOperation("title");
    };
    ...
    *type() {
        ...
        if (!text) {
            ...
            return yield this._noop();
        } else {
            yield this.evaluate_now(function (selector) {
                document.querySelector(selector).focus();
            }, selector);

            return yield this._invokeRunnerOperation("type", text);
        }
    };
    ...
    *url() {
        ...
        return yield this._invokeRunnerOperation("url");
    };
    ...
    *useragent(useragent) {
        ...
        return yield this._invokeRunnerOperation("useragent", useragent);
    };
    ...
    *viewport(width, height) {
        debug('.viewport()');
        return yield this._invokeRunnerOperation("size", width, height);
    };
    ...
    *waitUntilFinishLoad() {
        ...
        return yield this._invokeRunnerOperation("waitUntilFinishLoad");
    };
