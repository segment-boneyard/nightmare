...
class Nightmare {
        ...
        for (let propertyKey of propertyNames) {
            ...
            if (_.isArray(tink)) {
                if (tink.length === 2 && _.isFunction(tink[0]) && _.isFunction(tink[1])) {
                    ...
                    obj[propertyKey] = tink[1].bind(obj);
                }
            }
        }
    };
    ...
    initializeElectronActions() {
        ...
        for (let electronActionName of _.keys(Nightmare._electronActions)) {
            ...
            let p = this._invokeRunnerOperation(operationParams, electronActionName, String(Nightmare._electronActions[electronActionName]), id);
            promises.push(p);
        }
