...
function Nightmare(options) {
  ...
  this.queue(done => {
    ...
    chrome.init()
      ...
      .catch(err => {
        endInstance(this, () => this._rejectActivePromise(err));
      });
  });
}
...
Nightmare.prototype.queued = function (name, fn) {
  const self = this;
...
Nightmare.prototype.action = function () {
...
Nightmare.prototype.goto = function (url, headers) {
  ...
  this.queue(done => {
    ...
    this.chrome.Network.responseReceived(res => {
      // handle redirects
      this.chrome.debug('responseReceived');
